...
class IsaacValidator:
    ...
    def create_envs(self):
        for env_idx in range(self.batch_size):
            ...
            self.gym.set_actor_scale(env, object_handle, self.object_scale)  # scale the object
            ...
            if self.use_stiffness:
                robot_properties["stiffness"].fill(100000)
                robot_properties["damping"].fill(0)
            else:
    ...
    def set_actor_pose_dof(self, q):
        ...
        init_q, target_q = q.clone(), q.clone()
        ...
        for env_idx in range(len(self.envs)):
            ...
            dof_states_initial['pos'] = init_q[env_idx, self.urdf2isaac_order]
            ...
            dof_states_target['pos'] = target_q[env_idx, self.urdf2isaac_order]
    ...
    def run_sim(self):
        ...
        # force phase
        # for step in tqdm(range(self.steps_per_sec * 6), desc=f"[{self.robot_name}/{self.object_name}]"):
        ...
        object_transform[:, :3, :3] = R.from_quat(object_pose[:, 3:7]).as_matrix()
        ...
        robot_transform[:, :3, :3] = R.from_euler('XYZ', dof_states[:, 3:6]).as_matrix()

        robot_transform = np.linalg.inv(object_transform) @ robot_transform
        dof_states[:, :3] = torch.tensor(robot_transform[:, :3, 3])
        dof_states[:, 3:6] = torch.tensor(R.from_matrix(robot_transform[:, :3, :3]).as_euler('XYZ'))
